...
def main():
    ...
    for i in range(1):
        ...
        train_ds_og, val_ds_og, test_ds_og = utils_data.create_tf_dataset(train_files, train_labels, sample_rate= SAMPLE_RATE, 
                                                                ...
                                                                mode = 'train', gammatone = False, noise = True, noise_prob = 0.8, spec_augmentation = False, spec_prob = 0.8),\
                                    utils_data.create_tf_dataset(val_files, val_labels,sample_rate= SAMPLE_RATE, 
                                                                frame_length = FRAME_LENGTH, frame_step= FRAME_STEP,
                                                                mode = 'val', gammatone = False, noise = False, spec_augmentation = False),\
                                    utils_data.create_tf_dataset(test_files, test_labels, sample_rate= SAMPLE_RATE, 
                                                                frame_length = FRAME_LENGTH, frame_step= FRAME_STEP,
                                                                mode = 'test', gammatone = False, noise = False, spec_augmentation = False)
       ...
       #     _, adjacency_matrix, _ = utils_graph.create_adjacency_matrix(mfcc = example_mfcc, num_frames=N_FRAMES, label = example_label, mode='similarity', threshold= 0, cosine_window_thresh = 0.3,window_size_cosine= 3, window_size=5)
        ...
        # utils_data.visualize_single_waveform(example_wav, label = 1)
            ...
            train_ds = train_ds_og.map(lambda mfcc, wav, label: utils_graph.create_adjacency_matrix(mfcc, N_FRAMES, label, mode='cosine window',window_size_cosine = 5, n_dilation_layers= N_DILATION_LAYERS, window_size=5, threshold = 0.3))
            val_ds = val_ds_og.map(lambda mfcc, wav, label: utils_graph.create_adjacency_matrix(mfcc, N_FRAMES, label, mode='cosine window',window_size_cosine = 5, n_dilation_layers= N_DILATION_LAYERS, window_size=5, threshold = 0.3))
            test_ds = test_ds_og.map(lambda mfcc, wav, label: utils_graph.create_adjacency_matrix(mfcc, N_FRAMES, label,mode='cosine window',window_size_cosine = 5, n_dilation_layers= N_DILATION_LAYERS, window_size=5, threshold = 0.3))
      ...
      #      _ , mel_filters = utils_data.apply_mel_filterbanks(spectrogram)
        ...
        #  test_ds = test_ds.map(lambda mfcc, adjacency_matrices, label:  base_gnn.mfccs_to_graph_tensors_multi_nodes_for_dataset(mfcc, adjacency_matrices, label))
            ...
            if TRAINING:
                ...
                base_model.load_weights('best_model_weights.h5')
        ...
        # Optimize : dropout, learning rate, lr scheduler factor, batch size
