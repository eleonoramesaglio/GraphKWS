...
def main():
    ...
    for i in range(4):
        ...
        # This dataset we can use for our CNN implementation
        train_ds_og, val_ds_og, test_ds_og = utils_data.create_tf_dataset(train_files, train_labels, sample_rate= SAMPLE_RATE,
                                    ...
                                    utils_data.create_tf_dataset(val_files, val_labels,sample_rate= SAMPLE_RATE,
                                                                frame_length = FRAME_LENGTH, frame_step= FRAME_STEP,
                                                                mode = 'val', gammatone = True, noise = True, spec_augmentation = False),\
                                    utils_data.create_tf_dataset(test_files, test_labels, sample_rate= SAMPLE_RATE,
                                                                frame_length = FRAME_LENGTH, frame_step= FRAME_STEP,
                                                                mode = 'test', gammatone = True, noise = True, spec_augmentation = False)


        train_ds_og = train_ds_og.with_options(options)
        val_ds_og = val_ds_og.with_options(options)
        test_ds_og = test_ds_og.with_options(options)
        ...
        if MODE_MODEL == 'CNN':
         ...
         #   utils_metrics.visualize_confusion_matrix(y_pred, y_true, idx = i)
            ...
            #metrics_evaluation(y_pred, y_true, model_name = f"Model {i}") # TODO : doesn't work for cnn rn, but scores
            ...
            if REDUCED_NODE_REP_BOOL:
                train_ds_og = train_ds_og.map(lambda mfcc, wav, label : utils_graph.get_reduced_representation(mfcc,wav,label, k = REDUCED_NODE_REP_K, pooling_type = 'max'))
                val_ds_og = val_ds_og.map(lambda mfcc, wav, label : utils_graph.get_reduced_representation(mfcc,wav,label, k = REDUCED_NODE_REP_K, pooling_type = 'max'))
                test_ds_og = test_ds_og.map(lambda mfcc, wav, label : utils_graph.get_reduced_representation(mfcc,wav,label, k = REDUCED_NODE_REP_K, pooling_type = 'max'))
        ...
        #    _, adjacency_matrix, _ = create_adjacency_matrix(mfcc = example_mfcc, num_frames=N_FRAMES, label = example_label, mode='similarity', threshold= 0, cosine_window_thresh = 0.3,window_size_cosine= 3, window_size=5)
        ...
        # utils_data.visualize_single_waveform(example_wav, label = 1)
            ...
            # Create a new dataset with adjacency matrices (for GNNs)
            ...
            train_ds = train_ds_og.map(
                lambda mfcc, wav, label: utils_graph.create_adjacency_matrix(
                    mfcc, N_FRAMES, label, mode='cosine window', 
                    window_size_cosine = 25, n_dilation_layers= N_DILATION_LAYERS, 
                    window_size=5, threshold = 0.3
                ),
                num_parallel_calls=tf.data.AUTOTUNE
            )
            
            val_ds = val_ds_og.map(
                lambda mfcc, wav, label: utils_graph.create_adjacency_matrix(
                    mfcc, N_FRAMES, label, mode='cosine window',
                    window_size_cosine = 25, n_dilation_layers= N_DILATION_LAYERS, 
                    window_size=5, threshold = 0.3
       ...
       #     spectrogram, _ = get_spectrogram(wav, sample_rate = 16000)
        #    gam_filters = create_gammatone_filterbank(fft_size=FRAME_LENGTH)
        #  utils_data.visualize_filterbank(gam_filters, spectrogram = spectrogram, gammatone = True)
       #     _ , mel_filters = apply_mel_filterbanks(spectrogram)
        ...
        # utils_data.visualize_mfccs(example_mfcc, label = 1)
            ...
            # Graph test
         #   graph_example = mfccs_to_graph_tensors(example_mfcc, example_adjacency_matrix)
          ...
          #  networkx_graph = convert_tensor_to_networkx(graph_example)
          #  pos = node_layout(networkx_graph)
        ...
        #  test_ds = test_ds.map(lambda mfcc, adjacency_matrices, label:  base_gnn.mfccs_to_graph_tensors_multi_nodes_for_dataset(mfcc, adjacency_matrices, label))
         ...
         #   one_element_ds = train_ds.take(1)
           # graph_tensor, label = tf.data.experimental.get_single_element(one_element_ds)
            
            # Get the spec
          
          #  graphs_spec = one_element_ds[0]
            ...
            #plot_history(history, columns=['loss', 'sparse_categorical_accuracy'], idx = i)
            ...
            # Confusion matrix visualization
              ...
              y_pred, y_true = utils_metrics.get_ys(test_ds, base_model)
           # visualize_confusion_matrix(y_pred, y_true, idx = i)
