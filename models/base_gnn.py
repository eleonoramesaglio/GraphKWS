...
def curr_best_gnn_model(
    ...
    weighted edges + mfcc split + context node learning + 2 message passing layers // actually without context node learning, it seems to work better !
    ...
    for i in range(n_message_passing_layers):
        graph = tfgnn.keras.layers.GraphUpdate(
            ...
            context = tfgnn.keras.layers.ContextUpdate(
                {
                    "frames" : tfgnn.keras.layers.Pool(tfgnn.CONTEXT, "max", node_set_name = "frames")
                },
                next_state(next_state_dim, use_layer_normalization)
